    ...
    upload: function () {
        $("#update").click(function () {
            ...
            if (title != "" && classification != "" && tag != "" && content != "") {
                var tag = previewTag();
                var ariticle = { "title": title, "userId": userId, "invisibility": openId, "tag": tag, "UGC": escape(content), "Id": ariticleId, "treeNodeParentId": treeNodeId };
                var data = JSON.stringify(ariticle);
                $.post(baseUrl + "PersonalCenter/KnowledgeModify", { "ariticleJson": data }, function (cbdata) {
                    if (cbdata == "success") {
                        if (user.split("#")[3] != "3" && user.split("#")[3] != "2") {
                            var alert = new R2.Business.AlertWindow("修改成功，等待管理员审核！", {
                                LoadUrl: "PersonalCenter/PersonalCenter",
                            });
                        } else {
