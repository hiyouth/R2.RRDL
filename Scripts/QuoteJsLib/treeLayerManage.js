...
Rrteam.Control.Rrtree = OpenLayers.Class({
    ...
    */
    repeatCreateNode: function (data, ht) {
        ...
        for (var i = 0; i < data.length; i++)
            this.buildTreeNode(data[i], ht, parseInt(data[i].Deepth));
    },
    ...
    *创建树的节点，主要的方法 循环 执行
    */
    buildTreeNode: function (nodeData, ht, deep) {
        ...
        if (!nodeData.IsLeaf || nodeData.IsLeaf.toString().toLowerCase() == "false") {
            if (this.showCheckBox)
                ht.push("<img id='RrTree_", nodeData.Id, "-checkBox'  class='treeNodeIcon checkBox' src='", this.imagesUrl, "space.gif' />");
            else {
            ...
            if (this.model == "view" && this.showNodeTypeImg) {
                //图层预览图片uri
                //var layerImgUrl = "http://" + nodeData.NodeIp + ":" + nodeData.NodePort + "/igs/rest/mrms/layers?gdbps=" + encodeURI(encodeURI(nodeData.NodeGdbp));
                ////矢量图的gdbp
                //var layerUrl = "{\"NodeIp\":\"" + nodeData.NodeIp + "\",\"NodePort\":\"" + nodeData.NodePort + "\",\"NodeGdbp\":\"" + nodeData.NodeGdbp + "\"}";
                var stateClass = nodeData.NodeState == "hide" ? "treeNode_hide" : "treeNode_show";
                cs.push(stateClass);
                ht.push("<img class='treeNodeIcon' src='", this.imagesUrl, "space.gif' />");
                //ht.push("<img id='RrTree_", nodeData.Id, "-img' value='", layerUrl, "' class='treeNodeIcon layerState ", cs.join(' '), "' src='", this.imagesUrl, "space.gif' />");
                ht.push("<img class='noCursor' src='", this.imagesUrl, "space.gif'/>");
                //ht.push("<img class='treeNodeIcon layerImg' style='margin-left:3px;border:1px solid #C8C8C8; height:16px;width:18px;' src='", layerImgUrl, "' />");
            }
    ...
    */
    nodeClick: function (e, that, temp) {
        ...
        if (!temp.showCheckBox) {
            ...
            if ($(et).hasClass(coClassName) || controlImgObj.hasClass(coClassName)) {  //当前为收起状态
                ...
                if (controlImgObj.hasClass(coClassName)) {
                    ...
                    result = 'expand';
                    retunNodeId = $(et).attr("id").split('_')[1];
                    var returnObj = { "isLeaf": false, "nodeId": retunNodeId, "returnTitle": "" };
            ...
            else if ($(et).hasClass(exClassName) || controlImgObj.hasClass(exClassName)) {  //当前为展开状态
                ul.hide();
                result = 'collapse';
                if (controlImgObj.hasClass(exClassName)) {
                    temp.swapClass(controlImgObj, exClassName, coClassName);
                    retunNodeId = $(et).attr("id").split('_')[1];
                    var returnObj = { "isLeaf": false, "nodeId": retunNodeId, "returnTitle": "" };  //返回一个json对象，信息由节点的类型，节点的id, 操作提示
                    temp.nodeClickCallBack.call(temp.nodeClickCallBack, returnObj);  //回调
                }
                else {
                ...
                var returnObj = { "isLeaf": true, "nodeId": retunNodeId, "returnTitle": returnTitle };
                temp.nodeClickCallBack.call(temp.nodeClickCallBack, returnObj);  //回调
            }
            ...
            if ($(et).hasClass("checkBox")) {
                ...
                var oul = $("#" + ulId);
                temp.checkBoxClick(oul, state);          //对父节点、子节点进行处理

                //回调，将ul下的所有leaf节点的value返回
                var allLeafValue = [];
                allLeafValue = temp.getAllLeafValue(oul);
    ...
    *更改节点前checkBox
    */
    checkBoxClick: function (oul, state) {
        this.childrenCheck(oul, state);   //对子节点进行处理
        this.parentsCheck(oul, state);    //对父节点进行处理
    },
    ...
    */
    changeLayerState: function (oul) {
        var img = $(oul).find("img.layerState");
        if (img.size() > 0) {
            ...
            if (checkBox.hasClass("checkBox_che")) {
                ...
                for (var i = 0; i < img.size() ; i++) {
                    this.swapClass(img[i], "treeNode_show", "treeNode_hide");
                }
            }
        }
    },
    ...
    *判断节点前是否有复选框
    */
    isHaveUl: function (oul, ulid) {
        var ul = $(oul).find("ul." + ulid);
        if (ul.length > 0)
            return true;
        else
            return false;
    ...
    */
    childrenCheck: function (currentUl, state) {
        if (this.changeCheckBox(currentUl, state, 1) != false) {
            ...
            if (this.isHaveUl(currentUl, ulId)) {
                var ul = $(currentUl).find("ul." + ulId);         //将父节点的id作为class，方便找到直接 子ul，而不会找到 孙ul.
                var count = ul.size();
                for (var i = 0; i < count; i++) {
                    var _ul;
                    _ul = ul[i];
                    this.childrenCheck(_ul, state);
                }
            }
            else
                this.changeLayerState(currentUl);
        }
    },
    ...
    */
    parentsCheck: function (currentUl, state) {
        var pUl = $(currentUl).parent().parent();
        while (pUl) {
            if (this.changeCheckBox(pUl, state, 2) === false) {
                break;
            }
    ...
    */
    changeCheckBox: function (oul, state, type) {
        ...
        if ($(oul).hasClass("childrenExist")) {
            ...
            if (isNull != null) {
                var fse = checkBox.hasClass("checkBox_che");
    ...
    */
    getAllLeafValue: function (oul) {
        var allLeaf = oul.find("img.layerState");
        var allLeafValue = "{";
        var allLayerUri = "\"LayerUri\":";    //图层的uri
        var alllLeafText = "\"LayerName\":";  //图层的txt
        allLayerUri += "[";
        alllLeafText += "[";
        var size = $(allLeaf).size();
        if (size > 0) {
            for (var i = 0; i < size; i++) {
                var leafId = $(allLeaf[i]).attr("id");
                var textId = leafId.replace("img", "txt");
                allLayerUri += $("#" + leafId).attr("value") + ",";
                alllLeafText += "{";
                alllLeafText += "\"Name\":";
                alllLeafText += "\"" + $("#" + textId).attr("value") + "\"";
                alllLeafText += "},";
            }
            allLayerUri = allLayerUri.substring(0, allLayerUri.lastIndexOf(','));
            allLayerUri += "],";
            alllLeafText = alllLeafText.substring(0, alllLeafText.lastIndexOf(','));
            alllLeafText += "]";
            allLeafValue += allLayerUri;
            allLeafValue += alllLeafText;
            allLeafValue += "}";
        }
        return allLeafValue;
    },
